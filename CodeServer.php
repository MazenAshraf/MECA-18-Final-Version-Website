...
if(isset($_POST['SecRec'])){
    ...
    $row2 = mysql_fetch_row($comResult);
      ...
      //$committee1=$_POST['1st'];
    //$committee2=$_POST['2nd'];
//    $CommitteeQuery = " SELECT * FROM `Committees` where `committee`= '$committee1' OR `committee`='$committee2' ";
//    $comResult =mysql_query($CommitteeQuery);
//<?php while($row=mysql_fetch_array($comResult)):?>
<!--
 //   <?php  $available =$row['avail'];
   //         $newavailable=$available-1;
     // $committeename = $row['committee'];
     // $Updatequery = "UPDATE `timesInterview` SET `avalib`=$newavailable WHERE `Committee`= '$committeename' ";
      //$result2 = mysql_query($Updatequery);
//?> 
//<?php endwhile; ?>
//<?php   mysql_data_seek($result, 0);?>
-->
<!--    hn5lehaa requireeddd-->
...
/* //bdayet el platform'
$majorstudent="SELECT * From `students` Where `Major`='$major'";
$query4=mysql_query($studentquery);
$d=mysql_fetch_row($query4);
//$d['major']

$majorplatform=$_POST['major'];
$platformquery= "SELECT * FROM `platform` Where `major`='$majorplatform'";
$query5 = mysql_query($platformquery);
$r = mysql_fetch_row($query5);
if($d['Major']==$r['major'])
{

$_SESSION['major'] = "$majorplatform";
